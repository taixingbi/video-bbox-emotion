...
class HandleFrame:
    def __init__(self):
        ...
        self.emotional_classifier = load_model('model/model_v6_23.hdf5')
    
        self.class_labels= {0: 'ANGRY', 1: 'DISGUSTED', 2: 'FEAR', 3: 'HAPPY', 4: 'CALM', 5: 'SAD', 6: 'SURPRISED'}
    ...
    def emotional(self, face):
        ...
        if np.sum([face]) != 0.0:
            ...
            result_emotion= {   "HAPPY": float( preds[3] ),  
                                "CALM": float( preds[4] ),  
                                "SAD": float( preds[5] ),
                                "FEAR": float( preds[2] ),  
                                "ANGRY": float( preds[0] ),  
                                "DISGUSTED": float( preds[1] ),
                                "SURPRISED": float( preds[6])
                            }

            #update anxiety
            result_emotion['ANXIETY']=  result_emotion['FEAR'] + result_emotion['ANGRY'] + result_emotion['DISGUSTED']
    ...
    def visualization(self, Faces, img):
        ...
        for Face in Faces:
            ...
            labels= [   "HAPPY: "+ str(sentiment['HAPPY']*100)[:4]+ '%',  
                        ...
                        # "DISGUSTED: "+ str(sentiment['DISGUSTED']*100)[:4]+ '%',
